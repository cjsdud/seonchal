...
with t2:
    ...
    if st.button("진단하기", use_container_width=True):
        ...
        st.markdown(f"""<div class="verdict {hi}"><div class="v-grade {hi}">{lv}</div><div class="score-line"><span class="score-v">{score}</span><span class="score-c">/ 100점 · 예측 모델의 무응찰 위험 점수</span></div><div class="meter-wrap"><div class="meter"><div class="meter-pin" style="left:calc({max(2, min(98, score))}% - 1.5px)"></div></div><div class="meter-lab"><span>0 양호</span><span>30 주의</span><span>50 고위험</span><span>100</span></div></div><div class="v-fact" style="margin-top:.7rem">검증 실적 — 같은 등급을 받은 과거 공고 가운데 <b>{STAT[lv]}%</b>가 실제로 응찰 업체가 한 곳도 없었습니다.</div><div class="v-act"><b>권고 조치</b>{acts[lv]}</div></div>""", unsafe_allow_html=True)
