...
with t2:
    ...
    howto("등급은 어떻게 나오나요",
          ...
          "다만 화면에는 확률 대신 <span style='color:#16202C;font-weight:600'>등급과 과거 실적</span>을 "
          "보여드립니다. 확률값은 시장 상황이 바뀌면 어긋날 수 있지만, "
          "'이 등급을 받은 과거 공고의 78%가 실제로 응찰이 없었다'는 사실은 변하지 않기 때문입니다.")

    c1, c2 = st.columns([1, 1], gap="large")
    with c1:
        org = st.selectbox("수요기관", sorted(L["org_map"].keys()),
                           help="이 물품을 실제로 사용할 부대·기관입니다.")
    ...
    if st.button("진단하기", use_container_width=True):
        p = predict(org, item, biz, method, mfr, urgent, ind, rn)
        ...
        st.markdown(f"""        <div class="verdict {hi}"><div class="v-grade {hi}">{lv}</div><div class="v-fact">같은 등급을 받은 과거 공고 가운데 <b>{STAT[lv]}%</b>가 실제로 응찰 업체가 한 곳도 없었습니다.</div><div class="v-act"><b>권고 조치</b>{acts[lv]}</div></div>""", unsafe_allow_html=True)
        ...
        s = D[(D["수요기관"] == org) & (D["품목대분류"] == item)]
        if len(s):
            st.markdown(f'<div class="sec" style="margin-top:2rem">'
                        f'<span class="sec-n">참고 기록</span>'
                        f'<span class="sec-t">{org} · {item}</span>'
                        f'<div class="sec-d">과거 {len(s)}건 · 응찰이 없었던 비율 '
                        f'{s["무응찰"].mean()*100:.1f}% · 아래는 평소 응찰 규모입니다</div></div>',
                        unsafe_allow_html=True)
            st.markdown(tally(s["응찰수"].median()), unsafe_allow_html=True)
            st.markdown('<div class="note">작대기 하나가 응찰 업체 한 곳입니다. '
                        '다섯 곳마다 사선으로 묶었습니다. 응찰이 없으면 빈 칸만 남습니다.</div>',
                        unsafe_allow_html=True)
            st.dataframe(s[["공고일자", "공고명", "응찰수", "입찰진행상태값"]].tail(8)
                         .rename(columns={"입찰진행상태값": "결과", "응찰수": "응찰 업체 수"}),
                         use_container_width=True, hide_index=True)
        else:
            st.markdown('<div class="note">이 기관과 품목 조합의 과거 기록이 없습니다. '
                        '진단은 해당 품목 전체 실적을 기준으로 산출했습니다.</div>',
                        unsafe_allow_html=True)
